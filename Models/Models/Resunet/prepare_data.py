...
def segment_ct(x, imageLen, windowLen, n_moves):
    ...
    for i in range(n_moves):
        for j in range(n_moves):
            x_segmented[:, :, counterCrop] = x[int(i * imageLen / (n_moves + 1)):int(i * imageLen / (n_moves + 1) + windowLen),
                                            int(j * imageLen / (n_moves + 1)):int(j * imageLen / (n_moves + 1) + windowLen)]
            counterCrop += 1
    return x_segmented
...
def prepare_data(dataset_zip_dir, crossvalid_dir, numSubj, imageLen, windowLen, strideLen, n_moves, window_specs):
    ...
    datasetDir = Path(currentDir, 'ich_data', dataset_zip_dir.replace('.zip', '').replace(' (1)', ''))
    if os.path.isfile(str(Path(crossvalid_dir,'ICH_DataSegmentV1.pkl')))==False: #means the cross-validation folds was created before
        ...
        if os.path.exists(dataset_zip_dir)==True:
            ...
            subject_nums = np.array(subject_nums_shaffled)
            kf = KFold(n_splits=NumCV, shuffle=False)

            #reading images
            print("Dividing the data for the 5-fold cross-validation:")
            for cvI, (train_val_idx, test_idx) in enumerate(kf.split(subject_nums)):
                ...
                subjectNums_cvI_trainVal = subject_nums[train_val_idx]
                subjectNums_cvI_testing = subject_nums[test_idx]

                val_split = int(0.2 * len(subjectNums_cvI_trainVal))

                subjectNums_cvI_validate = subjectNums_cvI_trainVal[:val_split]
                subjectNums_cvI_train = subjectNums_cvI_trainVal[val_split:]

                counterI=0

                #Training CT scans
                for subID in subjectNums_cvI_train: # take only the 3 folds for training
                    sliceNums=hemorrhage_diagnosis_array[hemorrhage_diagnosis_array[:,0]==subID,1]

                    # Loading the CT scans and the masks
                    ct_scan, masks= load_ct_mask(datasetDir, subID + 49, window_specs)

                    NoHemorrhage=hemorrhage_diagnosis_array[hemorrhage_diagnosis_array[:, 0] == subID, 7]
                ...
                # Validation CT scans
                counterI = 0
                for subID in subjectNums_cvI_validate:  # take only the first fold for validation
                    sliceNums = hemorrhage_diagnosis_array[hemorrhage_diagnosis_array[:, 0] == subID, 1]
                ...
                # Testing CT scans
                for subID in subjectNums_cvI_testing:
                    sliceNums = hemorrhage_diagnosis_array[
                        hemorrhage_diagnosis_array[:, 0] == subID, 1]
                    ...
                    for sliceI in range(0, sliceNums.size):
                        ...
                        imsave(Path(crossvalid_dir,'CV' + str(cvI),'test','fullCT','image', str(subID)
                                          + '_' + str(counterI) + '.png'),np.uint8(ct_scan[:, :, sliceI]))
                        imsave(Path(crossvalid_dir,'CV' + str(cvI),'test','fullCT','label', str(subID)
                                          + '_' + str(counterI) + '.png'),np.uint8(masks[:, :, sliceI]))

                        counterCrop = 0
                        for i in range(n_moves * n_moves):
                            imsave(Path(crossvalid_dir, 'CV' + str(cvI), 'test','crops', 'image',
                                        str(subID)+ '' + str(counterI) + '' +
            ...
            print("Zipped dataset is not in the current directory. "
                  "Download the dataset from https://physionet.org/content/ct-ich/1.3.1/ then move it to same directory with main.ipynb")
    else:
        print("The dataset is already downloaded and the cross-validation folds were created!")
if __name__ == "__main__":
    prepare_data(
        dataset_zip_dir="computed-tomography-images-for-intracranial-hemorrhage-detection-and-segmentation-1.3.1 (1).zip",
