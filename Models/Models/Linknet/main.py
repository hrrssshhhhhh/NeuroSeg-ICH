...
from model import LinkNet  # Changed import statement
import imageio.v2 as imageio
...
class JaccardLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        eps = 1e-15
        intersection = torch.sum(y_pred * y_true, dim=(1, 2, 3))  # Compute intersection along all dimensions except batch dimension
        union = torch.sum(y_pred, dim=(1, 2, 3)) + torch.sum(y_true, dim=(1, 2, 3)) - intersection
        jac = (intersection + eps) / (union + eps)
        return torch.mean(1 - jac)  # Return the mean Jaccard loss over the batch
...
def Jaccard_img(y_true, y_pred):
    ...
    for i in range(y_true.shape[0]):
        if torch.sum(torch.tensor(y_true[i])) > 0:
            im1 = torch.tensor(y_true[i], dtype=torch.uint8)
            im2 = torch.tensor(y_pred[i], dtype=torch.uint8)
...
def train_model(model, criterion, optimizer, train_loader, val_loader, num_epochs, patience=5, model_save_path='model.pth'):
...
if __name__ == '__main__':
    ...
    for cvI in range(0, num_CV):
        ...
        model = LinkNet(num_classes=1).to(device)  # Change model initialization to ResUNet
            ...
            for sliceI in range(slicenum_s.size):
                ...
                for i in range(num_Moves):
                    for j in range(num_Moves):
                        ...
                        # Calculate the indices for the window in CTslicePredict
                        window_shape = windowI.shape
                        CTslicePredict_shape = CTslicePredict.shape
                        start_i = int(i * CTslicePredict_shape[0] / (num_Moves + 1))
                        end_i = start_i + window_shape[0]
                        start_j = int(j * CTslicePredict_shape[1] / (num_Moves + 1))
                        end_j = start_j + window_shape[1]
        
                        # Check if the window indices are within bounds of CTslicePredict
                        if end_i <= CTslicePredict_shape[0] and end_j <= CTslicePredict_shape[1]:
                            CTslicePredict[start_i:end_i, start_j:end_j] += windowI
                            windowOcc[start_i:end_i, start_j:end_j] += 1
                            counterCrop += 1
                        else:
                            print("Window is out of bounds")
                
                # Perform remaining operations and save the images
                windowOcc[windowOcc == 0] = 1
                ...
                kernel_closing_np = kernel_closing.squeeze().numpy()
                kernel_opening_np = kernel_opening.squeeze().numpy()

# Perform the closing operation
                img = cv2.morphologyEx(img, cv2.MORPH_CLOSE, kernel_closing_np)

# Perform the opening operation
                img = cv2.morphologyEx(img, cv2.MORPH_OPEN, kernel_opening_np)
