...
if __name__ == '__main__':
    ...
    for cvI in range(0, num_CV):
        ...
        model = UNetPlusPlus(n_channels=1, n_classes=1).to(device)
            ...
            for sliceI in range(slicenum_s.size):
                ...
                for i in range(num_Moves):
                    for j in range(num_Moves):
                        windowI = imread(Path(SaveDir_crops_cv, str(subjectNums_cvI_testing[subItest]) + '_' + str(sliceI) + '_' + str(counterCrop) + '.png'))
                        windowI = windowI / 255
                ...
                imsave(Path(SaveDir_full_cv, str(subjectNums_cvI_testing[subItest]) + '_' + str(sliceI) + '.png'), img)
                ...
                imsave(Path(SaveDir_cv, str(subjectNums_cvI_testing[subItest]) + '_' + str(sliceI) + '.png'), np.uint8(img))
